...
_sup_t, _sup_h = i18n.page_heading("en", "suppliers")
...
layout = dmc.Stack(
    ...
    children=[
        ...
        html.Div(id="suppliers-page-header", children=page_header(_sup_t, help=_sup_h)),
        dmc.SimpleGrid(
            cols={"base": 1, "md": 2},
            spacing="lg",
            children=[
                dmc.Card(
                    ...
                    children=[
                        ...
                        dmc.TextInput(id="sup-country", label="Country"),
                ...
                dmc.Card(
                    ...
                    padding="lg",
                    children=[
                        dmc.Text("Selected row", fw=600, mb="sm", size="sm", tt="uppercase", opacity=0.85),
...
@callback(
    Output("suppliers-page-header", "children"),
    Input("_pages_location", "pathname"),
    Input("locale-store", "data"),
)
def suppliers_page_header(pathname, loc):
    if normalize_path(pathname) != "/suppliers":
        raise PreventUpdate
    lang = i18n.normalize_lang(loc)
    t, h = i18n.page_heading(lang, "suppliers")
    return page_header(t, help=h)


@callback(Output("sup-grid", "rowData"), Input("_pages_location", "pathname"), Input("sup-version", "data"), Input("sup-refresh", "n_clicks"))
