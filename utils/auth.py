...
def session_user_id_for_write(s: Session) -> int | None:
    """Flask session user PK if that row exists and is active; else None (stale or logged out)."""
    uid = session.get("user_id")
    if not uid:
        return None
    u = s.get(models.User, int(uid))
    return int(uid) if u and u.is_active else None


def prune_invalid_session() -> None:
    """Clear cookie session when user_id no longer exists (e.g. after a user DB reset)."""
    uid = session.get("user_id")
    if not uid:
        return
